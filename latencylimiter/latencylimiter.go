...
func (l *latencyLimiter[R]) updateLimit(rtt time.Duration, inflight uint) uint {
	...
	if gradient >= 1 {
		...
		if float64(inflight) < l.limit/2 {
			fmt.Println(fmt.Sprintf("old limit=%0.2f, inflight=%d, shortRTT=%0.2f ms, longRTT=%0.2f ms",
				l.limit, inflight, shortRTT/1e6, longRTT/1e6))
			return uint(l.limit)
		}

		l.stablePeriods++
		if l.stablePeriods >= l.stableThreshold {
			queueSize = float64(l.growthStep)
			l.stablePeriods = 0 // Reset stable counter after a growth spurt
		}
	} else {
		l.stablePeriods = 0
	}
	...
	fmt.Println(fmt.Sprintf("%s new limit=%0.2f, inflight=%d, shortRTT=%0.2f ms, longRTT=%0.2f ms, queueSize=%0.0f, gradient=%0.2f", time.Now().Format("2006/01/02 15:04:05"),
