...
type PIDLimiter[R any] interface {
	AdaptiveLimiter[R]
...
/*
PIDLimiterBuilder builds PIDLimiter instances.

This type is not concurrency safe.
*/
type PIDLimiterBuilder[R any] interface {
	WithLogger(logger *slog.Logger) PIDLimiterBuilder[R]

	OnRejectionRateChanged(listener func(event RejectionRateChangedEvent)) PIDLimiterBuilder[R]

	Build() PIDLimiter[R]
}

// RejectionRateChangedEvent indicates a PIDLimiter's limit has changed.
type RejectionRateChangedEvent struct {
	OldRate float64
	NewRate float64
}

type pidConfig[R any] struct {
	delegate            AdaptiveLimiter[R]
	logger              *slog.Logger
	rateChangedListener func(event RejectionRateChangedEvent)
}

var _ PIDLimiterBuilder[any] = &pidConfig[any]{}

func NewPIDLimiter[R any](delegate AdaptiveLimiter[R]) PIDLimiter[R] {
	return NewPIDLimiterBuilder(delegate).Build()
}

func NewPIDLimiterBuilder[R any](delegate AdaptiveLimiter[R]) PIDLimiterBuilder[R] {
	return &pidConfig[R]{
		delegate: delegate,
	}
}

func (c *pidConfig[R]) WithLogger(logger *slog.Logger) PIDLimiterBuilder[R] {
	c.logger = logger
	return c
}

func (c *pidConfig[R]) OnRejectionRateChanged(listener func(event RejectionRateChangedEvent)) PIDLimiterBuilder[R] {
	c.rateChangedListener = listener
	return c
}

func (c *pidConfig[R]) Build() PIDLimiter[R] {
	return &pidLimiter[R]{
		pidConfig: c,
...
func (l *pidLimiter[R]) TryAcquirePermit() (Permit, bool) {
	return l.delegate.TryAcquirePermit()
}

func (l *pidLimiter[R]) Limit() int {
	return l.delegate.Limit()
}

func (l *pidLimiter[R]) Inflight() int {
...
func (l *pidLimiter[R]) BlockedExecutions() int {
